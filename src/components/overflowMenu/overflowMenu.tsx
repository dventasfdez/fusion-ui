...
const OverflowMenu: React.FC<IOverflowMenuProps> = ({ show, children, title, action, onClose, showCloseButton, onAction, className, parentRef, ...rest }) => {
    ...
    if (event && event.target) {
      if (
        ((overflowRef && overflowRef.current && !overflowRef.current.contains(event.target as Node) && !parentRef) ||
          (overflowRef && overflowRef.current && !overflowRef.current.contains(event.target as Node) && parentRef && parentRef.current && !parentRef.current.contains(event.target as Node))) &&
        show
      ) {
        onClose();
      }
  ...
  return show ? (
    <div ref={overflowRef} className={`overflow-menu ${className || ""}`} data-testid={rest["data-testid"] ? rest["data-testid"] : undefined} {...rest}>
      <div className="overflow-menu-header">
        {title && <div className="small-title mr3">{title}</div>}
        {action && (
          <div className="overflow-menu-action link_small" onClick={onAction} data-testid={rest["data-testid"] ? rest["data-testid"] + "-action-button" : undefined}>
            {action}
          </div>
        )}

        {showCloseButton && (
          <button type="button" className="overflow-menu-close" onClick={onClose} data-testid={rest["data-testid"] ? rest["data-testid"] + "-close-button" : undefined}>
