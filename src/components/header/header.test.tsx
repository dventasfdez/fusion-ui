...
const HeaderTest = (props: IHeaderProps) => (
  <Header {...props}>
    <HeaderLogo>
      <img
        src="https://upload.wikimedia.org/wikipedia/commons/thumb/9/92/Adecco_logo.svg/1200px-Adecco_logo.svg.png"
        alt="logo"
      />
    </HeaderLogo>
...
describe('Header desktop', () => {
  ...
  test('Component renders with language selector and match snapshot', () => {
    const component = renderer.create(<HeaderTest languages={_langs} />);
    const tree = component.toJSON();
    expect(tree).toMatchSnapshot();
  });
  test('Component renders with language selector and contact link and match snapshot', () => {
    const component = renderer.create(<HeaderTest languages={_langs} contact={_contact} />);
    const tree = component.toJSON();
    expect(tree).toMatchSnapshot();
  });
  test('Select language in header and click in other item (close overflow menu)', () => {
    ...
    const {container, getByTestId} = render(
      <HeaderTest
        ...
        languages={_langs}
        onClickLanguage={onChangeLanguageMock}
...
describe('Header mobile version', () => {
  ...
  test('Component renders mobile and match snapshot', () => {
    const component = renderer.create(<HeaderTest />);
    const tree = component.toJSON();
    expect(tree).toMatchSnapshot();
  });
  test('Component renders mobile with contact selector and match snapshot', () => {
    const component = renderer.create(<HeaderTest contact={_contact} />);
    const tree = component.toJSON();
    expect(tree).toMatchSnapshot();
  });
  test('Select language in header', () => {
    const onChangeLanguageMock = jest.fn();
    const onSearchMock = jest.fn();
  ...
  test('Click in item (close language selector) in header', () => {
    const onChangeLanguageMock = jest.fn();
    const onSearchMock = jest.fn();
    const {container, getByTestId} = render(
      <HeaderTest
  ...
  test('Click in item without href', () => {
    const onChangeLanguageMock = jest.fn();
    const onSearchMock = jest.fn();
    const {getByTestId} = render(
      <HeaderTest
