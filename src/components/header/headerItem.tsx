  ...
  const renderItem = () => {
          ...
          <TreeViewElement name={children} arrow="right" onClick={onClickItem} {...rest}>
            {options &&
              options.map((_opt: IHeaderItemOption, _idx: number) =>
                _opt.href ? (
                  <a data-testid={rest && rest["data-testid"] ? `${rest["data-testid"]}-option-${_idx}` : undefined} key={_opt.id || `header-item-option-${_idx}`} href={_opt.href}>
                    {_opt.label}
                  </a>
                ) : (
                  <div
    ...
    return options && options.length > 0 ? (
      <button
        ...
        disabled={disabled}
        onClick={onClickItem}
        className={`header-item ${selected ? "active" : ""} ${className || ""}`}
      >
        {children}
        {options && (
          <OverflowMenu
