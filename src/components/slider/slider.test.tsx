...
test('Double Slider pass onChange', () => {
  ...
  const {getByTestId} = render(
    <Slider double onChange={onChange} data-testid="slider" defaultValue={{min: 1, max: 20}} max={20} min={0} />
  );
  const slider = getByTestId('slider-input-test');
...
test.skip('Double Slider pass onChange with inside out boundaries', () => {
  ...
  const {getByTestId} = render(
    <Slider double onChange={onChange} data-testid="slider" defaultValue={{min: 15, max: 10}} max={20} min={0} />
  );
  const slider = getByTestId('slider-input-test');
