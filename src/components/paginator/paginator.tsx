...
const Paginator: React.FC<IPaginator> = (props) => {
  ...
  const onChange = (page: number) => {
    setCurrent(page);
    const chunk = chunks.findIndex((chunk) => chunk.find((_c) => _c === page));
    if (chunk !== -1) setChunk(chunk);
  ...
  if (arr && arr.length) {
    if (collapsed) {
      return (
        <div className={`paginator-collapsed ${className ?? ""}`} data-testid={rest && rest["data-testid"] ? rest["data-testid"] : undefined}>
          ...
          >
            {arr &&
              arr.length > 0 &&
              arr.map((num) => (
                <Option key={num + "select-item"} value={num} label={num.toString()} data-testid={rest && rest["data-testid"] ? `${rest["data-testid"]}-dropdown-item-${num}` : undefined}>
                  {num}
                </Option>
              ))}
          </Select>
